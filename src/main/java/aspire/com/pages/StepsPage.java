...
public class StepsPage extends FluentWebDriverPage {
	...
	public void scrollToElement(String element) throws IOException {
		...
		((JavascriptExecutor) driver).executeScript(
				"arguments[0].scrollIntoView();", driver.findElement(By
						.cssSelector(EnvirommentManager.getInstance()
								.getProperty(element))));
	}
	...
	public void scrollToElementAndStop(String element) throws IOException {
		WebDriver driver = getDriverProvider().get();
		((JavascriptExecutor) driver).executeScript(
				"arguments[0].scrollIntoView(false);", driver.findElement(By
						.cssSelector(EnvirommentManager.getInstance()
								.getProperty(element))));
	}
	...
	public void waitAnimationToDone() {
		...
		while (!(Boolean) (executor
				.executeScript("return $(\":animated\").length == 0;"))) {
	...
	public void wait(String element) throws IOException {
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(By
				.cssSelector(EnvirommentManager.getInstance().getProperty(element))));

	}

	public void waitAttributeToChange(String element, String attribute,
			String value) throws IOException {
		WebDriverWait wait = new WebDriverWait(getDriverProvider().get(), 20);
		final boolean result = findElement(
	...
	public boolean waitPresenceOfElement(String element) throws IOException {
		...
		try {
			wait.until(ExpectedConditions.presenceOfElementLocated(By
					.cssSelector(EnvirommentManager.getInstance().getProperty(
							element))));
		} catch (Exception e) {
			result = false;
		}
	...
	public void clickOnTheVisible(String element) throws IOException {
		Iterator<WebElement> day = findElements(
				By.cssSelector(EnvirommentManager.getInstance().getProperty(
						element))).iterator();
		while (day.hasNext()) {
	...
	public void enterValueToTextBox(String element, String value)throws IOException {
	  ...
	  public void enterDynamicValueToTextBox(String element, String dataEntry) throws IOException {
		wait(element);
		findElement(By.cssSelector(EnvirommentManager.getInstance().getProperty(
						element))).sendKeys(dataEntry + generateID());
	...
	public void clickOnElement(String element) throws IOException {
		...
		action.moveToElement(
				findElement(By.cssSelector(EnvirommentManager.getInstance()
						.getProperty(element)))).click().perform();
		// findElement(
		// By.cssSelector(EnvirommentManager.getInstance().getProperty(
		// element))).click();
		// WaitPageToLoad();
	...
	public boolean checkTheTitleOfThePages(String title) throws IOException {
		...
		if (findElement(
				By.cssSelector(EnvirommentManager.getInstance().getProperty(
						"Page_Title"))).getText().toString().contains(title)) {
			result = true;
		}
		return result;
	}

	public boolean compareTwoElement(String element, String value)
			throws IOException {
		...
		if (findElement(
				By.cssSelector(EnvirommentManager.getInstance().getProperty(
						element))).getText().toString().equalsIgnoreCase(value)) {
			result = true;
		}
		return result;
	}
	...
	public static String generateTimeStamp(String prefix) {
		...
		String token = new SimpleDateFormat("MM/dd/yyyy hh:mm").format(
				timestamp).toString();
	...
	public static String getCurrentTime() {
		java.util.Date date = new java.util.Date(); // Right now
		java.sql.Timestamp timestamp = new java.sql.Timestamp(date.getTime());
		String token = new SimpleDateFormat("MM/dd/yy hh:mm a").format(
				timestamp).toString();
	...
	public static String generateEmail(String prefix) {
		java.util.Date date = new java.util.Date(); // Right now
		java.sql.Timestamp timestamp = new java.sql.Timestamp(date.getTime());
		String token = new SimpleDateFormat("MMddyyyyhhmmss").format(timestamp)
				.toString();
	...
	public int calculateTheNumberOfItems(String items) throws IOException {
		...
		if (findElements(
				By.cssSelector(EnvirommentManager.getInstance().getProperty(
						"Last_page_Button"))).size() == 0) {
			if (findElements(
					By.cssSelector(EnvirommentManager.getInstance()
							.getProperty(items))).size() == 0) {
				num1 = 0;
			} else {
				num1 = findElements(
						By.cssSelector(EnvirommentManager.getInstance()
								.getProperty(items))).size();
			}
		} else {
	...
	public void hoverOverElement(String theElement) throws IOException {
		waitElementToDisappear("progressDialog");
		wait(theElement);
		WebElement elem = findElement(By.cssSelector(EnvirommentManager
				.getInstance().getProperty(theElement)));
	...
	public void resetAttribute(String element, String Attribute, String value) {
		...
		js.executeScript("arguments[0].setAttribute('" + Attribute + "', '"
				+ value + "')", setElement);
	}
	...
	public void SelectFromList(String element) throws IOException {
		...
		WaitElementToAppear(10 , element);
		 List<WebElement> allOptions = findElements(By.cssSelector(EnvirommentManager.getInstance().getProperty(element)));
	...
	public void TOCBrowsevolumes() throws IOException {
		...
		for (int i = 0; i < Volumes.size(); i++) {
			if (VerifyTheVisibiltyOfContentSimple("TOC_Articles")) {
				...
				Volumes_Years = findElements(By.cssSelector(EnvirommentManager.getInstance().getProperty("Volumes_Years")));
				...
				if(Volumes.get(i).getText().equals(null)){i++;}
				Volumes.get(i).click();
				try {
					WaitDOMToBeReady();
				} catch (Exception e) {
	...
	public void CheckPublishersLogo(String name) {
		...
		for (int i = 0; i < PublishersLogo.size(); i++) {
			if (PublishersLogo.get(i).getAttribute("href").contains(name)) {
				Assert.assertTrue(PublishersLogo.get(i).getAttribute("href").toLowerCase().contains(name.toLowerCase()));
				break;
			}
		}
	...
	public void PDFopenProperly() throws IOException {
		...
		String PDF_URL = findElement(By.cssSelector(EnvirommentManager.getInstance().getProperty("Access_PDF"))).getAttribute("href");
